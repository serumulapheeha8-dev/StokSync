...
export async function generateReceipt(contribution, member, group) {
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(27, 58, 87)
  doc.text('R' + parseFloat(contribution.amount || 0).toLocaleString('en-ZA'), pageW - 14, y + 14, { align: 'right' })
  ...
  // Save
  const filename = 'StokSync_Receipt_' + receiptNum + '_' + (member?.name || 'Member').replace(/\s+/g, '_') + '.pdf'
  doc.save(filename)
}